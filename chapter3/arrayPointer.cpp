...
int main() {
    ...
    int x2;
    int x3;
    int* a[3] = {&x1, &x2, &x3};//指针数组：a是指针
    std::cout << std::is_same_v<decltype(a), int*[3]> << '\n';

    int c[3];
    //int (*b)[3] = {&x1, &x2, &x3};error. a is pointer, cannot initialize through aggregate initialization.
    int (*b)[3] = &c;//数组的指针. Using array by initialization,
