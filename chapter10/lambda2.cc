...
int main() {
    ...
    auto lam1 = [] (int val) constexpr/*consteval也行*/ {
        return val + 1;
    };
    ...
    auto lam3 = [c = a + b/*捕获时计算*/] () {
        return c;
    }()/*IIFE，意思是在构造完lambda表达式之后，立即执行表达式*/;
    ...
    //避免该种问题，lambda表达式形参使用(const auto& p)
...
//    std::cout << lam7(5) << std::endl;
    ...
    //由于不能接着解析，不能确定赋值右侧的部分，也不能使用。
    //auto解析lam7类型

    //递归函数factorical如何解析。当遇到int factorical(int n)，编译器知道factorical是一个函数，输入int值，返回int值，
    //到了这一行， return n > 1 ? n * factorical(n - 1) : 1;自然可以解析

    //修改为如下
    auto lam7 = [](int n) {
        auto l_impl = [](int n, const auto& impl) -> int {
            return n > 1 ? n * impl(n - 1, impl) : 1;
        };
        return l_impl(n, l_impl);
    };
    std::cout << lam7(5) << std::endl;
    //auto类型判断是要整个语句解析完整，才知道。如何知道后面部分是lamb表达式，要整个解析完。lam7的函数体中没有调用lam7,
    //因此可以解析出lam7是一个lambda表达式。为什么内部可以解析出是lambda表达式：l_impl函数体中，也没有出现l_impl，
    //因此可以解析出l_impl是一个lambda表达式。return l_impl(n, l_impl);这一句，函数定义好了，正常调用也没问题。
    //才将l_impl变成递归。
    //是用了const auto& impl形成一个完整的表达式（函数模板表达式）。

    //如果最内层的lambda表达式类型 -> int 去掉，则为非法。return l_impl(n, l_impl);去掉，则为合法
    auto lam8 = [](int n) {
        auto l_impl = [](int n, const auto& impl)  {
            return n > 1 ? n * impl(n - 1, impl) : 1;
        };
//        return l_impl(n, l_impl);
    };
    ...
    //就不需要知道return n > 1 ? n * impl(n - 1, impl) : 1;返回的类型了
    //内层函数指定int类型，return l_impl(n, l_impl);返回类型也确定了，也都能work了。
