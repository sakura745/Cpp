...
int main() {
    ...
    //修改之后的代码过于长，不是很好书写，c++提供了 std::back_inserter
    ...
    //std::istream_iterator<int> b1;//legal
    //std::istream_iterator<int> b1();//illegal c++会将y解析为函数声明，而不是对象的构造。所以要用{}
    ...
    int res = std::accumulate(b, b1, 0);
    std::cout << res << std::endl;

    //ostream_iterator
    std::ostream_iterator<char> oo (std::cout);//{} ()在括号里面有参数的时候，都可以。但是没有参数要注意区分。防止c++解析错
