...
int main() {
    ...
    auto it = std::find(x.begin(), x.end(), 3);//使用迭代器
    std::cout << *it << std::endl;

    auto it2 = std::ranges::find(x/*为容器*/, 3);
//    auto it2 = std::ranges::find(x.begin(), x.end(), 3);//legal 也可以
...
//    std::cout << *it3 << std::endl;//illegal
    ...
    auto it4 = std::ranges::find(m, 3,
                                 &std::pair<const int, int>::second/*指向第二个元素的指针*/);
