...
//    int* y/*stack memory*/ = new int(2)/*heap分配了int大小的内存，把2保存到该内存上*/;//new函数返回一个int*，该表达式为复合表达式。
//    std::cout << *y << std::endl;
//    //... ...
//
...
//    //不会抛出异常
//    // 如何区分内存是否分配成功, y 是否是nullptr
//    int* y = new (std::nothrow) int(2);//如果nothrow去掉，则为错误的代码。因为内存开辟了，会执行else 失败了，会抛出异常，处理异常那段一直没用
//    if (y == nullptr) {
//        //.... 处理异常
//    } else {
//        std::cout << *y;
...
//    Str1 a;
//    std::cout << &a << std::endl;//也是00。说明对齐和内存是从heap还是stack分配无关，都会是对齐256.为了给cache，提升速度

    //placement delete对于内建类型来说，不需要delete关键字，自动销毁
    //placement delete对于类来说，通过调用析构函数来进行销毁
    char ch[sizeof(Str2)];
