...
namespace operatorOverload2 {
    ...
    struct Str5 {
        ...
        /*explicit*/ operator int/*operator int函数名*/() const/*防止const 类型转化失败*/ {
        //函数没有返回类型，或者说返回类型已经在函数名中了 int
            return val;
        }
    ...
    struct Str6 {
        ...
        //explicit bool特殊性：用于条件表达式时会隐式转换
...
using namespace operatorOverload2;
int main() {
    int x = 100;
    Str ptr(&x);
    ...
    *ptr = 101;
    ...
    //例如重载[]: int& operator[] (int id); id是一个对象，可以作为参数。
    //因此对于*和->重载的参数来说，需要特殊处理
    ...
    //运算符重载函数的全称写法
    std::cout << m2.operator->/*这是重载的指针*/()   ->/*这个是编译器翻译出来的*/val << std::endl;
    ...
    Str3 m3;
    //Str3的对象，访问了Str2的成员
    std::cout << m3->                                                         Str2Member/*Str2的数据成员*/ << std::endl;
    std::cout << m3.operator->()/*Str3的重载*/  .operator->()/*Str2的重载*/ ->Str2Member << std::endl;
...
//    std::cout << m7 << std::endl;//explicit operator bool() const 则illegal。Str6没有重载输出流
                                   //         operator bool() const 则legal。Str6被隐式转换为bool，输出0

    //当explicit存在时（explicit operator bool() const）
    auto var = m7 ? 1 : 0;
    ...
    std::cout << (m8 == 100)/*输入参数的顺序，可以运行*/ << std::endl;
    std::cout << (100 == m8)/*输入参数的相反顺序，仍可以运行(c20)。隐式交换操作数。c17又不行*/
              << std::endl;
    //也可以c20两个特性相结合
