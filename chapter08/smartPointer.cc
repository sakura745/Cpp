...
int main() {
   ...
   ///shared_ptr
    ...
    std::cout << *(yy1.get()) << std::endl;// y.get()返回一个int* 指针（因为是<int>），再解引用是对int* 指针解引用
    ...
    //make_shared分配一块内存写入99，同时分配给引用计数的内存。放的尽量近。是对shared_ptr的优化

    std::shared_ptr<int[]> ptr3(new int[5]);//c17
//    auto ptr4 = std::make_shared<int[]>(5);//c20
    //从compiler上看，只有gcc11.1支持，11.2-11.5都不支持
...
//    std::shared_ptr<int> p2(p);//illegal:double free
    std::cout << "--------------------" << std::endl;

    ///unique_ptr
    std::unique_ptr<int> xx(new int(100));//xx独占内存
