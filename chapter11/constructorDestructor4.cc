...
int main() {
    ...
    Str d = std::move(a);//相比与拷贝构造的很消耗资源，移动构造会速度加快。相对安全。需要定义拷贝构造，也要考虑构造移动构造

    //但是对于Str这个类，拷贝赋值函数和移动赋值函数在性能上区别不大。是因为这个类太简单，就一个这指针。

//    fun(3);//illegal:函数对名字重载解析，查找函数时，不会考虑delete，对于两个重载函数fun来说，fun(int)是完美匹配，fun(double)还需要隐式转换，
