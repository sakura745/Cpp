...
class Str {
    ...
    friend/*friend是单向的*/ class/*可有可无*/ Str2;//表示Str2是Str的朋友，Str的逻辑下，Str宣布谁是我的朋友。
...
//    fun1();//illegal: 类内定义的友元函数，作用域为类外全局域，但是编译器不会找到。好处是减轻编译器的负担，防止误用
    ...
    //ChatGPT：ADL 是 Argument-dependent Lookup 的缩写，意为“参数相关查找”。它是一种编程语言特性，常用于 C++ 和 D 等编程语言中。
    ...
    //使用 ADL 可以简化函数调用的代码，并促进代码的重用和扩展。但是它也可能会导致函数查找失败或查找到错误的函数定义，因此应该谨慎使用。
    Str val;
    fun1(val);
