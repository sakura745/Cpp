...
//情况二：不能在编译期求值。在编译期完成之后，开始运行期之前的这段期间，系统不能对X的初始值有任何的假设。系统会记录X的尺寸，
//将和其他尺寸类似的变量组织到一起，计算一共多大后，将尺寸信息置于可执行程序中。操作系统按照尺寸开辟内存空间，将开辟的空间中
//的数据都设置为0，空间交给程序使用，将该组织到一起的内存映射给各个对象。 ———— 零初始化
